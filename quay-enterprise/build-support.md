...
```sh
docker pull quay.io/coreos/quay-builder:v2.4.0
```
...
```sh
docker run --restart on-failure -e SERVER=wss://myquayenterprise -v /var/run/docker.sock:/var/run/docker.sock quay.io/coreos/quay-builder:v2.4.0
```
...
```sh
docker run --restart on-failure -e SERVER=wss://myquayenterprise -v /path/to/ssl:/usr/local/share/ca-certificates -v /var/run/docker.sock:/var/run/docker.sock quay.io/coreos/quay-builder:v2.4.0 /bin/sh -c "/usr/sbin/update-ca-certificates ; /quay-builder"
```
